...
beta <- read.csv(here("Data/beta_all_sites.csv"))#beta per storm per analyte per site
...
beta <- left_join(beta, storm_date, by = c("site", "stormID", "analyte"))
```
...
## curry
beta %>% filter(!is.na(analyte)) %>% ggplot(aes(x = as.Date(storm_date), y = beta, color = site))+
  geom_point()+
  facet_wrap(~analyte, scales = "free_y") + theme_minimal() +
  labs(title = "Beta per storm per analyte per site", x = "Storm Date")
